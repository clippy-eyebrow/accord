...
namespace { // private module-only namespace
    ...
    public:
        ...
        void poll(int timeout) { // timeout in milliseconds
            ...
            while (txbuf.size()) {
                int ret = kWrite(ptConnCtx, (char*)&txbuf[0], (int)txbuf.size(), 0);
                if (false) {}
                else if (ret < 0 && (socketerrno == SOCKET_EWOULDBLOCK || socketerrno == SOCKET_EAGAIN_EINPROGRESS)) {
                    break;
                }
    ...
    easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
        ...
        if (is_ssl) {
            ...
            if (!SSL_set_fd(ptConnCtx->sslHandle, (int)ptConnCtx->sockfd))
                ERR_print_errors_fp(stderr);
        ...
        }
        {
            ...
            int status;
            int i;
            snprintf(line, 256, "GET /%s HTTP/1.1\r\n", path); kWrite(ptConnCtx, line, (int)strlen(line), 0);
            if (port == 80) {
                snprintf(line, 256, "Host: %s\r\n", host); kWrite(ptConnCtx, line, (int)strlen(line), 0);
            }
            else {
                snprintf(line, 256, "Host: %s:%d\r\n", host, port); kWrite(ptConnCtx, line, (int)strlen(line), 0);
            }
            snprintf(line, 256, "Upgrade: websocket\r\n"); kWrite(ptConnCtx, line, (int)strlen(line), 0);
            snprintf(line, 256, "Connection: keep-alive, Upgrade\r\n"); kWrite(ptConnCtx, line, (int)strlen(line), 0);
            if (!origin.empty()) {
                snprintf(line, 256, "Origin: %s\r\n", origin.c_str()); kWrite(ptConnCtx, line, (int)strlen(line), 0);
            }
